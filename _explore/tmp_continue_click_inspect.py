...
with sync_playwright() as pw:
    ...
    info=p.evaluate("""() => {
      ...
      document.querySelectorAll('body *').forEach(el=>{
        ...
        if(text==='Continue Enhancing'||text==='Edit More'||text==='Download'){
          const r=el.getBoundingClientRect();
          if(r.width>0&&r.height>0) out.push({tag:el.tagName,text,cls:String(el.className),x:r.x,y:r.y,w:r.width,h:r.height});
        }
      });
      return out;
    }""")
    print('BUTTON INFO',info)
    # maybe click continue by coordinates
    if info:
      target=next((x for x in info if x['text']=='Continue Enhancing'),None)
      if target:
        p.mouse.click(target['x']+target['w']/2,target['y']+target['h']/2)
        p.wait_for_timeout(3000)
        print('AFTER COORD CLICK URL',p.url)
        print('AFTER COORD CLICK STATES',p.evaluate("""() => Array.from(document.querySelectorAll('[data-effect]')).map(el=>({effect:el.getAttribute('data-effect'),pressed:el.getAttribute('aria-pressed')}))"""))
        print('AFTER COORD CLICK BODY\n',p.locator('body').inner_text()[:3000])
        p.screenshot(path=str(OUT/'result_after_coordinate_continue_click.png'), full_page=False)
    b.close()
