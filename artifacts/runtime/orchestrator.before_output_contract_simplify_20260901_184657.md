...
用户自然语言中的限定词必须先拆解为任务声明字段，禁止把执行限定误扩展为产物协议变更：

- `task_intent`：要执行的阶段 / 角色，例如 page-map-sync、test-case-design、test-writing、review。
- `scope`：需求文档、目标 URL、页面、状态、用例范围。
- `environment`：base_url、测试服 / 预部署、账号态、素材、验证码等项目级覆盖。
- `execution_constraints`：执行方式约束，例如“纯 MCP”“只探索”“先验证 MCP”“不写脚本”。
- `artifact_contract`：由 runtime / role skill 固定决定，包括标准产物名、目录、frontmatter schema、正文结构、Gate、状态机；除非用户明确要求“改变产物格式 / 另存为 / 不走 gate”，自然语言限定词不得改变该契约。

限定词作用范围：

| 用户说法 | 只应该影响 | 不应该影响 |
|---|---|---|
| 纯 MCP 探索 | 页面交互和证据采集方式，记录为 `exploration_driver: playwright_mcp_only` | `sync.md` 文件名、sync 章节格式、Gate、page_map 版本化规则 |
| 只探索 | 只执行 page-map-sync 并停在 sync gate | 跳过 `pending_review`、输出非标准 artifact |
| 先验证 MCP | 增加 MCP connectivity precheck 并记录证据 | 新建特殊报告文件、改变后续接力路径 |
| 测试服域名改成 xxx | 本任务 `base_url` / `target_url` 覆盖 | 修改 `PROJECT.md` 全局事实，除非用户明确要求 |
| 按用例对照实际页面 | 覆盖矩阵按用例 / AC 逐条编号 | 脱离用例盲扫或把未覆盖伪装成 covered |
| 两个页面进行探索 | `scanned_pages` 和 coverage matrix 包含这两个页面 | 合并成非标准产物或跳过单页 page_map 版本化 |

执行前应在 `state.md` 固化任务声明中的关键字段，至少包含：`task_type`、`execution_mode`、`exploration_driver`（如适用）、`target_url`、`artifacts.sync`、`next_step`。若用户指定“纯 MCP”，仍必须写标准 `artifacts/<task_id>/sync.md`，正文按同类历史 `sync.md` 编号章节风格输出。
...
7. 用户确认归档：只做复制、registry 更新、archive.md；归档脚本复跑要另外生成 `allure-results-archive/` 与 `allure-report-archive/`。暂不归档：写 archive.md `skipped_by_user`，可 completed。
