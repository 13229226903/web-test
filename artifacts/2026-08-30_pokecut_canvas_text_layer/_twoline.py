...
with sync_playwright() as p:
    ...
    print('count', ta.count())
    if ta.count():
        ta.fill("line1\nline2")
        page.wait_for_timeout(1500)
        print('value after fill', repr(ta.input_value()))
        # blur by pressing Escape or clicking blank? clicking blank deselects? maybe click elsewhere not blank? use keyboard Escape
        page.keyboard.press("Escape")
        page.wait_for_timeout(800)
    # dump textarea again
    data=page.evaluate("""() => { const out=[]; document.querySelectorAll('textarea').forEach(el=>{const r=el.getBoundingClientRect(); if(r.width>0||r.height>0) out.push({cls:el.className, text:el.value, x:Math.round(r.x), y:Math.round(r.y)});}); return out; }""")
