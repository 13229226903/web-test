...
with sync_playwright() as p:
    ...
    data=page.evaluate("""() => { const out=[]; document.querySelectorAll('button,div,span,p').forEach(el=>{const r=el.getBoundingClientRect(); const t=(el.textContent||'').trim(); if(t==='Text'||t==='Picture'||t==='Adjust Layer'){out.push({tag:el.tagName,cls:el.className,text:t,x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)});}}); return out; }""")
