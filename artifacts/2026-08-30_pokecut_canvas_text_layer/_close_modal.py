...
with sync_playwright() as p:
    ...
    page.locator("input.hexInput").fill("#0000ff"); page.keyboard.press("Enter"); page.wait_for_timeout(1000)
    print('start hex visible', page.locator('input.hexInput').is_visible())
    # try clicking outside modal at panel area x=1200,y=500
    page.mouse.click(1200,500); page.wait_for_timeout(1000)
    print('after outside click hex visible', page.locator('input.hexInput').is_visible())
    # if still visible, try Escape twice
    page.keyboard.press("Escape"); page.wait_for_timeout(500); page.keyboard.press("Escape"); page.wait_for_timeout(1000)
    print('after esc2 hex visible', page.locator('input.hexInput').is_visible())
    browser.close()
