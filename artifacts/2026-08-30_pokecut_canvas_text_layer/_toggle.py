...
with sync_playwright() as p:
    ...
    print('initial panel visible', panel_visible())
    # click toolbar Adjust by coordinate
    toolbar=page.locator("button", has_text="Adjust").all()
    for b in toolbar:
        try:
            r=b.bounding_box()
            if r and 410<r['y']<440:
                print('toolbar adjust', r)
                page.mouse.click(r['x']+r['width']/2, r['y']+r['height']/2)
                break
        except: pass
    page.wait_for_timeout(1500)
    print('after toolbar adjust panel visible', panel_visible())
    # click blank canvas area (left center, not on panel)
    page.mouse.click(900, 300)
    page.wait_for_timeout(1500)
    print('after blank canvas click panel visible', panel_visible())
    b.close()
