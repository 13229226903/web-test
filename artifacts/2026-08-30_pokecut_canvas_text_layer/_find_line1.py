...
with sync_playwright() as p:
    ...
    print('body has line1', 'line1' in body, 'line2', 'line2' in body)
    print(body[:1500].replace('\n',' | '))
    # search elements containing line1
    data=page.evaluate("""() => { const out=[]; document.querySelectorAll('div,span,textarea,p').forEach(el=>{const t=(el.textContent||'').trim(); if(t.includes('line1')){const r=el.getBoundingClientRect(); out.push({tag:el.tagName, cls:el.className, text:t.slice(0,100), x:Math.round(r.x), y:Math.round(r.y), w:Math.round(r.width), h:Math.round(r.height)});}}); return out; }""")
