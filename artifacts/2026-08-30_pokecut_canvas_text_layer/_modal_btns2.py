...
with sync_playwright() as p:
    ...
    data=page.evaluate("""() => { const out=[]; document.querySelectorAll('button').forEach(b=>{const r=b.getBoundingClientRect(); const t=(b.textContent||'').trim(); if(r.width>0&&r.height>0&&r.x>1500&&r.y>830){out.push({text:t,cls:b.className,html:b.innerHTML.slice(0,200),x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)});}}); return out.slice(0,30); }""")
