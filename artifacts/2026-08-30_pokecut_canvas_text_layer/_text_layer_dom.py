...
with sync_playwright() as p:
    ...
    data=page.evaluate("""() => {
      ...
      document.querySelectorAll('*').forEach(el=>{
        if((el.textContent||'').trim()==='Add Text'){
          const r=el.getBoundingClientRect();
          out.push({tag:el.tagName, cls:el.className, x:Math.round(r.x), y:Math.round(r.y), w:Math.round(r.width), h:Math.round(r.height), html:el.outerHTML.slice(0,500)});
        }
      });
      return out;
    }""")
