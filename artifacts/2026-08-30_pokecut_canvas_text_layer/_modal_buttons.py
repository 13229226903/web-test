...
with sync_playwright() as p:
    ...
    data=page.evaluate("""() => { const out=[]; document.querySelectorAll('button,div,span').forEach(el=>{const r=el.getBoundingClientRect(); const t=(el.textContent||'').trim().replace(/\\s+/g,' '); if(r.width>0&&r.height>0&&r.x>1500&&r.y>830&&t&&t.length<30) out.push({tag:el.tagName,text:t,cls:el.className,x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)});}); const seen=new Set(); const res=[]; for(const o of out){const k=o.tag+'|'+o.text+'|'+o.x+'|'+o.y; if(!seen.has(k)){seen.add(k);res.push(o);}} return res.slice(0,50); }""")
