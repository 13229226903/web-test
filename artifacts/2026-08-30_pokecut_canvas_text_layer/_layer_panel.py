...
with sync_playwright() as p:
    ...
    print('after layer click body', body[:1500].replace('\n',' | '))
    data=page.evaluate("""() => { const out=[]; document.querySelectorAll('button,div,span,p').forEach(el=>{const r=el.getBoundingClientRect(); const t=(el.textContent||'').trim(); if(t && r.width>0 && r.height>0 && r.x>=1000 && r.y>400){out.push({tag:el.tagName,text:t.slice(0,50),x:Math.round(r.x),y:Math.round(r.y)});}}); const seen=new Set(); const res=[]; for(const o of out){const k=o.tag+'|'+o.text+'|'+o.x+'|'+o.y; if(!seen.has(k)){seen.add(k);res.push(o);}} return res; }""")
