...
with sync_playwright() as p:
    ...
    print('palette modal open: hex visible', page.locator('input.hexInput').is_visible(), 'wrapper', page.locator('.color-picker-wrapper').count())
    page.locator("input.hexInput").fill("#0000ff"); page.keyboard.press("Enter"); page.wait_for_timeout(1200)
    print('after Enter: hex visible', page.locator('input.hexInput').is_visible(), 'wrapper', page.locator('.color-picker-wrapper').count())
    page.keyboard.press("Escape"); page.wait_for_timeout(1000)
    print('after Escape: hex visible', page.locator('input.hexInput').is_visible(), 'wrapper', page.locator('.color-picker-wrapper').count())
    browser.close()
