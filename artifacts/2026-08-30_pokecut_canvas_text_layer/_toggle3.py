...
with sync_playwright() as p:
    ...
    print('panel initial', panel_visible())
    # click toolbar Adjust via locator click (first button whose text exactly Adjust and y<440)
    btn=page.locator("button", has_text="Adjust").first
    print('btn count', page.locator("button", has_text="Adjust").count())
    try:
        r=btn.bounding_box(); print('first adjust box', r)
        btn.click()
    except Exception as e:
        print('click err', e)
    page.wait_for_timeout(1500)
    print('panel after locator click', panel_visible())
    # click toolbar Adjust by coordinate again with page.mouse, using the toolbar one
    ...
    for bb in toolbar:
        try:
            ...
            if r and r['y']<440:
                ...
                print('mouse clicked', r)
                break
        except: pass
    page.wait_for_timeout(1500)
    print('panel after mouse click', panel_visible())
    browser.close()
