...
with sync_playwright() as p:
    ...
    data=page.evaluate("""() => { const out=[]; document.querySelectorAll('img').forEach((el,i)=>{const r=el.getBoundingClientRect(); if(r.width>50&&r.height>50) out.push({i,src:el.src.slice(0,80),x:Math.round(r.x),y:Math.round(r.y),w:Math.round(r.width),h:Math.round(r.height)});}); return out; }""")
